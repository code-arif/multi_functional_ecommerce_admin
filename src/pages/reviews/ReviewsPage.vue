<template>
  <div>
    ...
    <DataTable :items="reviews" :columns="columns" :loading="loading" :pagination="pagination" @page="load" empty-icon="⭐">
      <template #default="{ item }">
        <td class="table-cell">
          <div class="flex items-center gap-1">
            <span v-for="i in 5" :key="i" class="text-sm" :class="i<=item.rating?'text-yellow-400':'text-gray-200'">★</span>
        ...
        <td class="table-cell text-right">
          <div class="flex items-center justify-end gap-1">
            <button v-if="item.status!=='approved'" @click="approve(item.id)" class="text-xs bg-green-100 text-green-700 hover:bg-green-200 px-2.5 py-1.5 rounded-lg font-semibold transition">✓ Approve</button>
            <button v-if="item.status!=='rejected'" @click="reject(item.id)" class="text-xs bg-red-100 text-red-600 hover:bg-red-200 px-2.5 py-1.5 rounded-lg font-semibold transition">✕ Reject</button>
            <button @click="confirmDelete(item)" class="p-1.5 rounded-lg text-red-400 hover:bg-red-50"><TrashIcon class="w-4 h-4" /></button>
          </div>
        </td>
      </template>
    </DataTable>
    <ConfirmModal :show="!!deleteTarget" title="Delete Review" message="Delete this review permanently?" :loading="deleting" @confirm="doDelete" @cancel="deleteTarget=null" />
  </div>
</template>
<script setup>
...
# Banners Page
cat > /home/claude/ecommerce-admin/src/pages/banners/BannersPage.vue << 'VUEEOF'
<template>
  <div>
    <PageHeader title="Banners & Sliders" subtitle="Manage homepage banners and hero sliders">
      <button @click="openForm()" class="btn-primary"><PlusIcon class="w-4 h-4" /> Add Banner</button>
    </PageHeader>
    <DataTable :items="banners" :columns="columns" :loading="loading" empty-icon="🖼️" empty-text="No banners yet">
      <template #default="{ item }">
        <td class="table-cell">
          <div class="flex items-center gap-3">
            <img v-if="item.image_url" :src="item.image_url" class="w-16 h-10 object-cover rounded-lg bg-gray-100 shrink-0" />
            <div v-else class="w-16 h-10 bg-gray-100 rounded-lg shrink-0 flex items-center justify-center text-gray-300">🖼️</div>
            <div>
        ...
        <td class="table-cell"><span class="badge badge-blue text-[10px]">{{ item.position?.replace('_',' ') }}</span></td>
      ...
      <div v-if="showForm" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative bg-white rounded-2xl p-6 w-full max-w-lg animate-in" style="box-shadow:0 20px 60px rgba(0,0,0,0.15)">
          ...
          <form @submit.prevent="save" class="space-y-4">
            <div><label class="label">Title *</label><input v-model="form.title" required class="input" /></div>
            ...
            <div><label class="label">Link URL</label><input v-model="form.link" class="input" placeholder="https://..." /></div>
            <div><label class="label">Button Text</label><input v-model="form.button_text" class="input" placeholder="Shop Now" /></div>
            <div><label class="label">Banner Image {{ editing?'(leave empty to keep current)':'' }} *</label>
              <input type="file" :required="!editing" accept="image/*" @change="e=>imageFile=e.target.files[0]" class="input" /></div>
            <div class="grid grid-cols-2 gap-3">
              <div><label class="label">Starts At</label><input v-model="form.starts_at" type="datetime-local" class="input" /></div>
              <div><label class="label">Ends At</label><input v-model="form.ends_at" type="datetime-local" class="input" /></div>
            </div>
            ...
            <div class="flex gap-3 pt-2">
              <button type="submit" :disabled="saving" class="btn-primary flex-1 justify-center">{{ saving?'Saving...':'Save Banner' }}</button>
    ...
    <ConfirmModal :show="!!deleteTarget" title="Delete Banner" :message="`Delete '${deleteTarget?.title}'?`" :loading="deleting" @confirm="doDelete" @cancel="deleteTarget=null" />
  </div>
</template>
<script setup>
