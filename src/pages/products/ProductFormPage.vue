    ...
    <form @submit.prevent="save" class="space-y-6">
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 w-full">
              ...
              <div>
                ...
                <div class="quill-wrapper" :class="{ 'quill-error': errors.description }">
                  <QuillEditor v-model:content="form.description" content-type="html" :options="quillOptions"
                    theme="snow" placeholder="Detailed product description..." />
                </div>
          ...
          <div v-if="form.type === 'variable'" class="card p-5">
            ...
            <div v-for="(attr, ai) in form.attributes" :key="ai" class="border border-gray-200 rounded-xl p-4 mb-3">
              <div class="flex items-center gap-3 mb-3">
                ...
                <button type="button" @click="form.attributes.splice(ai, 1)"
        ...
        <div class="space-y-5">
          ...
          <div class="card p-5">
            <div class="flex items-center justify-between mb-4">
              <h3 class="font-bold text-gray-900">Product Gallery</h3>
            ...
            <!-- Upload Box -->
            <div
              class="border-2 border-dashed border-gray-200 rounded-2xl p-6 text-center cursor-pointer hover:border-[#4CAF50] hover:bg-green-50 transition"
              @click="$refs.galleryInput.click()">
              <ImagePlus class="w-8 h-8 text-gray-400 mx-auto mb-2" />
              <p class="text-sm font-medium text-gray-600">Upload Product Images</p>
              <p class="text-xs text-gray-400 mt-1">PNG, JPG, WEBP (Max 5MB)</p>
            </div>

            <input ref="galleryInput" type="file" multiple accept="image/*" class="hidden" @change="onGalleryChange" />

            <!-- Grid -->
            <div v-if="imagePreviews.length" class="grid grid-cols-2 sm:grid-cols-3 gap-4 mt-5">
              <div v-for="(img, index) in imagePreviews" :key="index"
                class="relative group rounded-2xl overflow-hidden border border-gray-200 bg-white shadow-sm">
                <!-- Image -->
                <img :src="img" class="w-full h-32 object-cover" />

                <!-- Primary Badge -->
                <div v-if="uploadedImages[index]?.is_primary"
                  class="absolute top-2 left-2 bg-[#2E7D32] text-white text-[10px] px-2 py-1 rounded-full font-semibold shadow">
                  Primary
                </div>

                <!-- Uploading Loader -->
                <div v-if="uploadedImages[index]?.uploading"
                  class="absolute inset-0 bg-black/50 flex items-center justify-center text-white text-xs">
                  Uploading...
                </div>
                ...
                <div class="absolute top-2 right-2 flex gap-2 opacity-0 group-hover:opacity-100 transition">
                  ...
                  <button type="button" @click="setPrimary(index)"
                    class="w-9 h-9 rounded-full bg-white shadow text-gray-700 flex items-center justify-center hover:bg-yellow-400 hover:text-white transition"
                    title="Set Primary">
                    <Star class="w-4 h-4" />
                  </button>
                  ...
                  <button type="button" @click="removeImage(index)"
                    class="w-9 h-9 rounded-full bg-red-500 shadow text-white flex items-center justify-center hover:bg-red-600 transition"
                    title="Remove">
          ...
          <div class="card p-5">
            ...
            <div class="flex gap-2">
              <input v-model="newTag" @keyup.enter="addTag" class="input-sm flex-1"
