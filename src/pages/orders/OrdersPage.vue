    ...
    <div class="flex gap-2 mb-5 overflow-x-auto pb-1">
      <button v-for="tab in statusTabs" :key="tab.value" @click="statusFilter=tab.value; load(1)"
        ...
        <span v-if="tab.count" class="text-[10px] px-1.5 py-0.5 rounded-full"
          :class="statusFilter===tab.value ? 'bg-white/20 text-white' : 'bg-gray-100 text-gray-500'">{{ tab.count }}</span>
      </button>
    </div>

    <DataTable :items="orders" :columns="columns" :loading="loading" :pagination="pagination"
      searchable search-placeholder="Order number, name, phone..." empty-icon="📋" empty-text="No orders found"
      ...
      <template #default="{ item }">
        ...
        <td class="table-cell"><StatusBadge :value="item.payment_status" /></td>
        <td class="table-cell">
...
cat > /home/claude/ecommerce-admin/src/pages/orders/OrderDetailPage.vue << 'VUEEOF' <template>
    <div class="max-w-5xl">
