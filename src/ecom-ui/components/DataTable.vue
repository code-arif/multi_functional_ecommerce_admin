<template>
  <div class="card overflow-hidden">
    ...
    <div class="overflow-x-auto">
      <table class="w-full">
        <thead class="bg-gray-50/80">
          <tr>
            <th v-for="col in columns" :key="col.key" class="table-header" :class="col.class">{{ col.label }}</th>
          </tr>
        </thead>
        <tbody>
          <tr v-if="loading">
    ...
    <div v-if="pagination && pagination.last_page > 1"
      ...
      <p class="text-xs text-gray-500">Showing {{ pagination.from }}–{{ pagination.to }} of {{ pagination.total }}</p>
      <div class="flex items-center gap-1">
        <button @click="$emit('page', pagination.current_page - 1)" :disabled="pagination.current_page === 1"
          class="px-2 py-1.5 rounded-lg border border-gray-300 text-gray-500 hover:bg-white disabled:opacity-30 transition">
          <ChevronLeftIcon class="w-4 h-4" />
        </button>
        <button v-for="page in pagination.last_page" :key="page" @click="$emit('page', page)"
          :class="[
        ...
        <button @click="$emit('page', pagination.current_page + 1)" :disabled="pagination.current_page === pagination.last_page"
          class="px-2 py-1.5 rounded-lg border border-gray-300 text-gray-500 hover:bg-white disabled:opacity-30 transition">
...
defineProps({
  ...
  loading: Boolean, pagination: Object, searchable: Boolean,
  searchPlaceholder: String, emptyText: String, emptyIcon: String,
})
